...
interface Notification {
  ...
  time: string;
  read: boolean;
  type: 'payment' | 'system' | 'approval' | 'invoice' | 'receipt';
  created_at: string;
}
...
const AccountantLayout = ({ children }: { children?: ReactNode }) => {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'receipt': return <Receipt size={14} className="text-green-500" />;
      case 'approval': return <Bell size={14} className="text-yellow-500" />;
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel('accountant-notifications')
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const navItems: NavItem[] = [
    ...
    {
      title: 'Receipts',
      href: '/portal/accountant/receipts',
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="text-left hidden md:block">
                  ...
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Accountant</p>
                </div>
