...
const PropertyManager: React.FC<PropertyManagerProps> = ({
  ...
  const stats = {
    ...
    active: properties.filter((p) => p.status === "active" && p.is_active).length,
    assignedManagers: new Set(properties.map((p) => p.manager_id).filter(Boolean)).size,
    totalUnits: properties.reduce((sum, p) => sum + (p.total_units || 0), 0),
    occupiedUnits: properties.reduce((sum, p) => sum + (p.occupied_units || 0), 0),
    availableUnits: properties.reduce((sum, p) => sum + (p.available_units || 0), 0),
    cities: new Set(properties.map((p) => p.city).filter(Boolean)).size,
    totalRevenue: properties.reduce((sum, p) => sum + ((p.monthly_rent || 0) * (p.occupied_units || 0)), 0),
  };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setNewProperty({...newProperty, security_deposit: parseFloat(e.target.value) || 0})}
                      />
                    </div>
                  </div>
