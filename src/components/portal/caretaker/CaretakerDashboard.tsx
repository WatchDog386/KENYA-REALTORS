...
const CaretakerDashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Get caretaker info with property details
      const { data: caretaker, error: caretakerError } = await supabase
        ...
        .select(`
          ...
          assignment_date,
          performance_rating,
          profile:profiles!user_id(id, first_name, last_name, email, phone, avatar_url),
          property:properties(id, name, location),
          property_manager:profiles!property_manager_id(first_name, last_name)
        `)
        .eq('user_id', user.id)
        .maybeSingle();

      if (caretakerError) {
        console.error('Error fetching caretaker:', JSON.stringify(caretakerError, null, 2));
      ...
      if (caretaker.property_id) {
        try {
          ...
          // Calculate stats
          const pendingIssues = (maintenance || []).filter(m => m.status === 'pending' || m.status === 'in_progress').length;
