...
const MakePaymentPage: React.FC = () => {
  ...
  const navigateToTenantPostPaymentEntry = async () => {
    if (!user?.id) {
    ...
    try {
      ...
      if (!leaseAgreementState.isSigned) {
        navigate("/portal/tenant/lease-agreement");
        return;
      }
    } catch (error) {
      console.warn("Unable to verify lease agreement status after payment:", error);
    }
  ...
  const handlePaystackPaymentSuccess = async (transactionRef: string, paymentData: any) => {
    ...
    try {
      ...
      if (!receiptPropertyId || !receiptUnitId) {
        ...
        if (onboardingFlowUnlocked) {
          await navigateToTenantPostPaymentEntry();
        } else {
      ...
      if (shouldOpenDashboard) {
        ...
        setTimeout(() => {
          void navigateToTenantPostPaymentEntry();
        }, 1000);
      } else {
        if (onboardingInvoiceResult.paidInvoices.length > 0) {
