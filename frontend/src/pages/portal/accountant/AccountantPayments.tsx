...
interface MaintenanceProcurement {
  ...
  supplier_invoice_notes?: string;
  supplier_invoice_image_url?: string;
...
const AccountantPayments = () => {
  ...
  const approveProcurement = async (report: MaintenanceProcurement) => {
    try {
      ...
      const approvedCost = Number(
        report.supplier_invoice_amount || report.actual_cost || report.cost_estimate || 0
      );
                  ...
                  {procurements.map((report) => (
                    <TableRow key={report.id}>
                      ...
                      <TableCell>
                        ...
                        {report.supplier_invoice_number && (
                          <div className="text-xs text-blue-700">Invoice: {report.supplier_invoice_number}</div>
                        )}
                      </TableCell>
                      <TableCell>
                        KES {Number(report.supplier_invoice_amount || report.actual_cost || report.cost_estimate || 0).toLocaleString()}
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline" className={
                          report.status === 'paid'
                            ? 'bg-green-50 text-green-700 border-green-200'
                            : report.status === 'supplier_submitted'
