...
const SuperAdminUtilitiesManager = () => {
  ...
  const scopedTenants = isAllPropertiesWorkspace
    ? tenantsWithReadings
    : tenantsWithReadings.filter((tenant) => tenant.property_id === selectedPropertyId);

  const scopedBillingSummary = useMemo(() => {
    const pendingInvoices = scopedTenants.filter((tenant) => tenant.status !== 'paid').length;
    const paidInvoices = scopedTenants.filter((tenant) => tenant.status === 'paid').length;
    const totalDue = scopedTenants.reduce((sum, tenant) => sum + Number(tenant.total_due || 0), 0);
    const collected = scopedTenants.reduce((sum, tenant) => {
      const due = Number(tenant.total_due || 0);
      const outstanding = Math.max(0, due - Number(tenant.utility_total || 0));
      return sum + Math.max(0, due - outstanding);
    }, 0);
    const overdue = scopedTenants.filter((tenant) => Number(tenant.total_due || 0) > 0 && tenant.status !== 'paid').length;
    const activeUnits = new Set(scopedTenants.map((tenant) => tenant.unit_id || `${tenant.tenant_id}-${tenant.unit_number}`)).size;
    ...
    return {
      ...
      activeUnits,
      collectionRate,
      qaPending: scopedFirstPaymentCandidates.length + scopedTransitionCandidates.length,
    };
  }, [scopedTenants, scopedFirstPaymentCandidates, scopedTransitionCandidates]);
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="min-h-screen space-y-5 bg-[#eef2f7] p-4 md:p-6 lg:p-8"
    >
      {/* Header */}
      <div className="rounded-xl border border-[#d4dde8] bg-[#f7f9fc] px-4 py-4 shadow-[0_4px_14px_rgba(15,39,65,0.06)]">
        <div className="text-[11px] font-semibold tracking-[0.16em] text-[#6f839b] uppercase">Workspace View</div>
        <div className="mt-1 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-4xl font-bold tracking-tight text-[#0f2741]">Billing and Invoicing</h1>
            <p className="mt-1 text-sm text-[#5f7690]">Monitor billing workload, invoice throughput, and collection health.</p>
          </div>
          <Badge className="border border-[#2a6ea1] bg-[#1f6fa4] text-white uppercase tracking-wider">Billing Ops</Badge>
        </div>
        <div className="mt-3 inline-flex items-center rounded border border-[#9fd8b2] bg-[#e8f7ee] px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.12em] text-[#1f7f46]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        <div className="rounded-xl border border-[#d4dde8] bg-white px-4 py-3 shadow-[0_4px_14px_rgba(15,39,65,0.06)] transition-all duration-200 hover:-translate-y-[1px] hover:shadow-[0_10px_22px_rgba(15,39,65,0.1)]">
          <div className="text-[11px] uppercase tracking-[0.14em] text-[#6f839b] font-semibold">Incoming Invoices</div>
          <div className="text-4xl font-bold text-[#0f2741] mt-1">{scopedBillingSummary.pendingInvoices}</div>
          <div className="text-xs text-[#6f839b] mt-1">Current snapshot</div>
        </div>
        <div className="rounded-xl border border-[#d4dde8] bg-white px-4 py-3 shadow-[0_4px_14px_rgba(15,39,65,0.06)] transition-all duration-200 hover:-translate-y-[1px] hover:shadow-[0_10px_22px_rgba(15,39,65,0.1)]">
          <div className="text-[11px] uppercase tracking-[0.14em] text-[#6f839b] font-semibold">Assigned Today</div>
          <div className="text-4xl font-bold text-[#0f2741] mt-1">{scopedBillingSummary.paidInvoices}</div>
          <div className="text-xs text-[#6f839b] mt-1">Current snapshot</div>
        </div>
        <div className="rounded-xl border border-[#d4dde8] bg-white px-4 py-3 shadow-[0_4px_14px_rgba(15,39,65,0.06)] transition-all duration-200 hover:-translate-y-[1px] hover:shadow-[0_10px_22px_rgba(15,39,65,0.1)]">
          <div className="text-[11px] uppercase tracking-[0.14em] text-[#6f839b] font-semibold">QA Pending</div>
          <div className="text-4xl font-bold text-[#0f2741] mt-1">{scopedBillingSummary.qaPending}</div>
          <div className="text-xs text-[#6f839b] mt-1">Current snapshot</div>
        </div>
        <div className="rounded-xl border border-[#d4dde8] bg-white px-4 py-3 shadow-[0_4px_14px_rgba(15,39,65,0.06)] transition-all duration-200 hover:-translate-y-[1px] hover:shadow-[0_10px_22px_rgba(15,39,65,0.1)]">
          <div className="text-[11px] uppercase tracking-[0.14em] text-[#6f839b] font-semibold">Active Units</div>
          <div className="text-4xl font-bold text-[#0f2741] mt-1">{scopedBillingSummary.activeUnits}</div>
          <div className="text-xs text-[#6f839b] mt-1">Current snapshot</div>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        <div className="rounded-xl border border-[#26a8ae] bg-[#26a8ae] text-white px-4 py-3 shadow-[0_6px_16px_rgba(38,168,174,0.3)] transition-all duration-200 hover:-translate-y-[1px]">
          <div className="text-3xl font-black">{formatKES(scopedBillingSummary.totalDue)}</div>
          <div className="text-lg font-semibold">Total Due</div>
          <div className="mt-2 text-xs uppercase tracking-[0.12em] text-white/85">Live team view</div>
        </div>
        <div className="rounded-xl border border-[#2eaa52] bg-[#2eaa52] text-white px-4 py-3 shadow-[0_6px_16px_rgba(46,170,82,0.3)] transition-all duration-200 hover:-translate-y-[1px]">
          <div className="text-3xl font-black">{formatKES(scopedBillingSummary.collected)}</div>
          <div className="text-lg font-semibold">Collected</div>
          <div className="mt-2 text-xs uppercase tracking-[0.12em] text-white/85">Live team view</div>
        </div>
        <div className="rounded-xl border border-[#e8b232] bg-[#e8b232] text-[#3d2d05] px-4 py-3 shadow-[0_6px_16px_rgba(232,178,50,0.28)] transition-all duration-200 hover:-translate-y-[1px]">
          <div className="text-3xl font-black">{scopedBillingSummary.collectionRate}%</div>
          <div className="text-lg font-semibold">Collection Rate</div>
          <div className="mt-2 text-xs uppercase tracking-[0.12em] text-[#665211]">Live team view</div>
        </div>
        <div className="rounded-xl border border-[#de435c] bg-[#de435c] text-white px-4 py-3 shadow-[0_6px_16px_rgba(222,67,92,0.28)] transition-all duration-200 hover:-translate-y-[1px]">
          <div className="text-3xl font-black">{scopedBillingSummary.overdue}</div>
