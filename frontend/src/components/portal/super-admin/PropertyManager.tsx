...
const PropertyManager: React.FC = () => {
  ...
  const getLivePropertySnapshot = async (property: Property): Promise<Property> => {
    ...
    const liveCountByTypeId = new Map<string, number>();
    const breakdownByTypeName = new Map<
      string,
      {
        name: string;
        total_units: number;
        tiers: Map<string, { price_per_unit: number; units_count: number }>;
      }
    >();
    ...
    units.forEach((unit: any) => {
      ...
      if (typeId) {
        liveCountByTypeId.set(typeId, (liveCountByTypeId.get(typeId) || 0) + 1);
      }

      const breakdownKey = typeName.toLowerCase();
      const group = breakdownByTypeName.get(breakdownKey) || {
        name: typeName,
        total_units: 0,
        tiers: new Map<string, { price_per_unit: number; units_count: number }>(),
      };

      group.total_units += 1;

      const tierKey = String(resolvedPrice);
      ...
      breakdownByTypeName.set(breakdownKey, group);
    });

    const mappedTypes = configuredTypes.map((typeRow: any) => {
      const typeId = String(typeRow.id || '');
      const liveUnitsCount = liveCountByTypeId.get(typeId);
      return {
        id: String(typeRow.id || normalizeTypeName(typeRow)),
        name: normalizeTypeName(typeRow),
        units_count: units.length > 0 ? (liveUnitsCount || 0) : Number(typeRow.units_count || 0),
        price_per_unit: Number(typeRow.price_per_unit || 0),
        sample_image_url: String(typeRow.sample_image_url || ''),
      };
    });

    const mappedTypeIds = new Set(mappedTypes.map((item) => String(item.id)));
    const derivedMissingTypes = units
      .filter((unit: any) => !mappedTypeIds.has(String(unit.unit_type_id || '')))
      .reduce((acc: Array<any>, unit: any) => {
        const unitTypeRow = Array.isArray(unit.property_unit_types)
          ? unit.property_unit_types[0]
          : unit.property_unit_types;
        const typeName = normalizeTypeName(unitTypeRow);
    ...
    const unitPriceBreakdown = Array.from(breakdownByTypeName.values())
      .map((group) => ({
        name: group.name,
        total_units: group.total_units,
        tiers: Array.from(group.tiers.values()).sort((a, b) => a.price_per_unit - b.price_per_unit),
      }))
      .sort((a, b) => a.name.localeCompare(b.name));

    if (units.length === 0 && configuredTypes.length > 0) {
      configuredTypes.forEach((typeRow: any) => {
        const typeName = normalizeTypeName(typeRow);
        const existing = unitPriceBreakdown.find((group) => group.name.toLowerCase() === typeName.toLowerCase());
        if (!existing) {
          unitPriceBreakdown.push({
            name: typeName,
            total_units: Number(typeRow.units_count || 0),
            tiers: [
              {
                price_per_unit: Number(typeRow.price_per_unit || 0),
    ...
    const fallbackTotalUnits = mappedTypes.reduce((sum: number, row: any) => sum + Number(row.units_count || 0), 0);
    const fallbackExpectedIncome = mappedTypes.reduce(
      (sum: number, row: any) => sum + (Number(row.units_count || 0) * Number(row.price_per_unit || 0)),
      0,
    );

    return {
      ...property,
      total_units: units.length > 0 ? units.length : fallbackTotalUnits,
      expected_income: units.length > 0 ? expectedIncome : fallbackExpectedIncome,
      number_of_floors: floorSet.size || property.number_of_floors || 1,
      property_unit_types: [...mappedTypes, ...derivedMissingTypes],
      unit_price_breakdown: unitPriceBreakdown,
    };
  };
  ...
  const handleEditProperty = async (property: Property) => {
    const liveProperty = await getLivePropertySnapshot(property);
    const configuredTypes = (liveProperty.property_unit_types || []) as Array<{
      id?: string;
      name?: string;
      units_count?: number;
      price_per_unit?: number;
    ...
    const liveBreakdown = ((liveProperty as any).unit_price_breakdown || []) as Array<{
      name: string;
      total_units: number;
      tiers: Array<{ price_per_unit: number; units_count: number }>;
    }>;

    const normalizeName = (value: string) => String(value || '').trim().toLowerCase();
    const usedConfiguredIds = new Set<string>();

    const findExactConfigured = (name: string, price: number) => {
      return configuredTypes.find((row) => {
        const rowId = String(row.id || '');
        if (!rowId || usedConfiguredIds.has(rowId)) return false;
        return normalizeName(String(row.name || '')) === normalizeName(name)
          && Number(row.price_per_unit || 0) === Number(price || 0);
      });
    };

    const findNameConfigured = (name: string) => {
      return configuredTypes.find(
        (row) => normalizeName(String(row.name || '')) === normalizeName(name),
      );
    };

    const unitsFromBreakdown = liveBreakdown.flatMap((group) => {
      const sortedTiers = Array.isArray(group.tiers)
        ? [...group.tiers].sort((a, b) => Number(a.price_per_unit || 0) - Number(b.price_per_unit || 0))
        : [];

      return sortedTiers.map((tier) => {
        const exactConfigured = findExactConfigured(group.name, Number(tier.price_per_unit || 0));
        const imageSource = exactConfigured || findNameConfigured(group.name);

        if (exactConfigured?.id) {
          usedConfiguredIds.add(String(exactConfigured.id));
        }

        return {
          id: exactConfigured?.id,
          name: group.name,
          units_count: Number(tier.units_count || 0),
          price_per_unit: Number(tier.price_per_unit || 0),
          sample_image_url: String(imageSource?.sample_image_url || ''),
        };
      });
    });

    const fallbackUnits = configuredTypes.map((row) => ({
      id: row.id,
      name: String(row.name || ''),
      units_count: Number(row.units_count || 0),
      price_per_unit: Number(row.price_per_unit || 0),
      sample_image_url: String(row.sample_image_url || ''),
    }));

    const editUnits = unitsFromBreakdown.length > 0
      ? unitsFromBreakdown
  ...
  const totalUnits = safeProperties.reduce((sum: number, p: Property) => sum + (p.total_units || 0), 0);
  const totalIncome = safeProperties.reduce((sum: number, p: Property) => sum + (p.expected_income || 0), 0);
  ...
  return (
    <div className="font-['Poppins','Segoe_UI',sans-serif] text-[#243041] p-4 md:p-6 bg-[#d7dce1] min-h-screen">
      ...
      {!propertyForUnits && (
      <>
      <section className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-[#bcc3cd] pb-4">
          <div>
            <h1 className="text-[34px] font-bold leading-none text-[#1f2937]">Properties</h1>
            <p className="mt-1 text-[12px] font-medium text-[#5f6b7c]">
              Manage your property portfolio, staffing, units, and pricing.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Dialog open={showAddPropertyForm} onOpenChange={setShowAddPropertyForm}>
                ...
                <Button className="inline-flex h-9 items-center gap-2 rounded-md border border-[#2f3d51] bg-[#2f3d51] px-3 text-[11px] font-semibold uppercase tracking-wide text-white transition-colors hover:bg-[#243041]">
                    <Plus className="w-5 h-5 mr-2" />
                    Add Property
                </Button>
                </DialogTrigger>
                <DialogContent className="w-screen max-w-none h-screen p-0 gap-0 overflow-hidden bg-[#d7dce1] shadow-none rounded-none border-0">
                     {/* Header - Clean White */}
                    ...
                    <div className="p-6 overflow-y-auto max-h-[calc(100vh-140px)] space-y-8 bg-white">
      ...
      <Dialog open={showViewProperty} onOpenChange={setShowViewProperty}>
        <DialogContent className="w-screen max-w-none h-screen p-0 gap-0 overflow-hidden bg-[#d7dce1] shadow-none rounded-none border-0">
          ...
          {selectedProperty && (
            <>
              <div className="px-4 py-2 border-b border-[#bcc3cd] bg-[#eef1f4] sticky top-0 z-20">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="h-9 w-9 border border-[#c7cdd6] bg-white flex items-center justify-center">
                      <Eye className="w-4 h-4 text-[#154279]" />
                    </div>
                    <div>
                      <h2 className="text-[24px] font-bold leading-none text-[#1f2937]">Property Overview</h2>
                      <p className="text-[#5f6b7c] text-[12px] font-medium mt-1">Full profile, staff assignment, and unit breakdown.</p>
                    </div>
                  </div>
                  <div className="text-right text-xs text-[#6a7788]">
                    <p className="uppercase tracking-wider font-semibold">Viewing</p>
                    <p className="text-[#1f2937] font-bold mt-1">{selectedProperty.name}</p>
                  </div>
                </div>
              </div>

              <div className="h-[calc(100vh-145px)] overflow-y-auto p-4 bg-[#d7dce1]">
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                  <div className="xl:col-span-5 space-y-6">
                    <Card className="border border-[#bcc3cd] bg-[#eef1f4] rounded-none shadow-none overflow-hidden">
                      <div className="h-60 bg-[#d7dce1] border-b border-[#c7cdd6]">
                        {selectedProperty.image_url ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-[#e1e6ec]">
                            <Building className="w-16 h-16 text-[#94a0ad]" />
                          </div>
                        )}
                      </div>
                      <CardContent className="p-5 space-y-4">
                        <div>
                          <h3 className="text-2xl font-bold text-[#1f2937]">{selectedProperty.name}</h3>
                          <div className="mt-2 flex items-center gap-2 text-[#324156]">
                            <MapPin className="w-4 h-4 text-[#5f6b7c]" />
                            <span className="font-medium">{selectedProperty.location}</span>
                          </div>
                          <Badge className="mt-3 bg-[#154279] text-white px-3 py-1 rounded-none">{selectedProperty.type || 'Apartment'}</Badge>
                        </div>

                        {selectedProperty.description && (
                          <div className="pt-1">
                            <p className="text-xs font-bold uppercase tracking-wider text-[#6a7788] mb-2">Description</p>
                            <p className="text-[#243041] leading-relaxed">{selectedProperty.description}</p>
                          </div>
                        )}

                        {selectedProperty.amenities && (
                          <div className="pt-1">
                            <p className="text-xs font-bold uppercase tracking-wider text-[#6a7788] mb-2">Amenities</p>
                            <p className="text-[#243041] leading-relaxed">{selectedProperty.amenities}</p>
                          </div>
                        )}
                      </CardContent>
                    </Card>

                    <Card className="border border-[#bcc3cd] bg-[#eef1f4] rounded-none shadow-none">
                      <CardHeader className="pb-3">
                        <CardTitle className="text-lg font-bold text-[#1f2937]">Property Metrics</CardTitle>
                        <CardDescription className="text-[#5f6b7c]">Quick operational and revenue indicators.</CardDescription>
                      </CardHeader>
                  ...
                  <div className="xl:col-span-7 space-y-6">
                    <Card className="border border-[#bcc3cd] bg-[#eef1f4] rounded-none shadow-none">
                      <CardHeader className="pb-3">
                        <CardTitle className="text-lg font-bold text-[#1f2937]">Assigned Staff</CardTitle>
                        <CardDescription className="text-[#5f6b7c]">Current workforce assignments for this property.</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-3">
                    ...
                    {((selectedProperty as any).unit_price_breakdown?.length > 0 || selectedProperty.property_unit_types?.length > 0) && (
                      <Card className="border border-[#bcc3cd] bg-[#eef1f4] rounded-none shadow-none overflow-hidden">
                        <CardHeader className="pb-3 border-b border-[#bcc3cd] bg-[#eef1f4]">
                          <CardTitle className="text-lg font-bold text-[#1f2937]">Unit Type Breakdown</CardTitle>
                          <CardDescription className="text-[#5f6b7c]">Unit mix and exact rent tiers from Unit Management.</CardDescription>
                        </CardHeader>
                        <CardContent className="p-0 divide-y divide-[#d4dae2]">
                          {((selectedProperty as any).unit_price_breakdown || []).map((group: any) => (
                            <div key={`group-${group.name}`} className="px-5 py-4 space-y-2">
                              <div className="flex items-center justify-between">
                                <p className="font-semibold text-[#1f2937] uppercase tracking-wide">{group.name}</p>
                                <p className="text-xs font-semibold text-[#5f6b7c]">{Number(group.total_units || 0)} units total</p>
                              </div>
                              <div className="space-y-2">
                                {(group.tiers || []).map((tier: any, tierIndex: number) => (
                                  <div key={`${group.name}-tier-${tier.price_per_unit}-${tierIndex}`} className="flex items-center justify-between border border-[#d4dae2] bg-white px-3 py-2">
                                    <p className="text-[12px] font-semibold text-[#243041]">Count: {Number(tier.units_count || 0)}</p>
                                    <p className="text-[12px] font-bold text-[#1f2937]">KES {(Number(tier.price_per_unit) || 0).toLocaleString()}</p>
                                  </div>
                                ))}
                              </div>
                            </div>
                          ))}
                          {(selectedProperty as any).unit_price_breakdown?.length === 0 &&
                            (selectedProperty.property_unit_types || []).map((unitType: any, idx: number) => (
                              <div key={`${unitType.id || unitType.name}-${idx}`} className="px-5 py-4 flex items-center justify-between">
                                <div>
                                  <p className="font-semibold text-[#1f2937]">{unitType.name}</p>
                                  <p className="text-xs text-[#5f6b7c]">{unitType.units_count} units</p>
                                </div>
                                <p className="font-bold text-[#1f2937]">KES {(Number(unitType.price_per_unit) || 0).toLocaleString()}</p>
                              </div>
                            ))}
                        </CardContent>
                      </Card>
              ...
              <DialogFooter className="px-6 py-4 border-t border-[#bcc3cd] bg-[#eef1f4] sticky bottom-0 z-20">
                <Button
                  variant="outline"
                  onClick={() => setShowViewProperty(false)}
                  className="h-9 rounded-none border border-[#b6bec8] bg-white px-4 text-[11px] font-semibold uppercase tracking-wide text-[#465870] hover:bg-[#f5f7fa]"
                >
                  Close
                </Button>
                <Button
                  ...
                  className="h-9 rounded-none border border-[#154279] bg-[#154279] px-4 text-[11px] font-semibold uppercase tracking-wide text-white hover:bg-[#10335f]"
                >
                  Edit Property
                </Button>
              </DialogFooter>
        ...
        <DialogContent className="w-screen max-w-none h-screen p-0 gap-0 overflow-hidden bg-[#d7dce1] shadow-none rounded-none border-0">
          ...
          <div className="px-4 py-2 border-b border-[#bcc3cd] bg-[#eef1f4] sticky top-0 z-20">
            <DialogHeader className="p-0">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="h-9 w-9 border border-[#c7cdd6] bg-white flex items-center justify-center">
                    <Building className="w-4 h-4 text-[#154279]" />
                  </div>
                  <div>
                    <DialogTitle className="text-[24px] font-bold leading-none text-[#1f2937]">Edit Property</DialogTitle>
                    <DialogDescription className="text-[#5f6b7c] text-[12px] font-medium mt-1">
                ...
                <div className="text-right text-xs text-[#6a7788]">
                  ...
                  <p className="text-[#1f2937] font-bold mt-1">{selectedProperty?.name || formData.name || 'Property'}</p>
                </div>
              </div>
            </DialogHeader>
          </div>
          ...
          <div className="h-[calc(100vh-145px)] overflow-y-auto p-4 bg-[#d7dce1]">
              ...
              <div className="xl:col-span-5 space-y-6">
                <Card className="border border-[#bcc3cd] bg-[#eef1f4] rounded-none shadow-none">
                  <CardHeader className="pb-4">
                    <CardTitle className="text-lg font-bold text-[#1f2937]">Property Profile</CardTitle>
                    <CardDescription className="text-[#5f6b7c]">Core identity and location details.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      <Label className="text-[#6a7788] font-semibold text-[11px] uppercase tracking-wide">Property Name</Label>
                      <Input
                        value={formData.name}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, name: e.target.value })}
                        placeholder="e.g. Sunrise Apartments"
                    ...
                    <div className="space-y-2">
                      <Label className="text-[#6a7788] font-semibold text-[11px] uppercase tracking-wide">Location</Label>
                      <div className="relative">
                        <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6a7788] w-4 h-4" />
                        <Input
                          value={formData.location}
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, location: e.target.value })}
                          placeholder="e.g. Westlands, Nairobi"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label className="text-[#6a7788] font-semibold text-[11px] uppercase tracking-wide">Property Type</Label>
                        <Select value={formData.type} onValueChange={(val: string) => setFormData({ ...formData, type: val })}>
                          <SelectTrigger className={PROFILE_INPUT_CLASS}>
                            <SelectValue placeholder="Select type" />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="Apartment">Apartment</SelectItem>
                      ...
                      <div className="space-y-2">
                        <Label className="text-[#6a7788] font-semibold text-[11px] uppercase tracking-wide">Floors</Label>
                        <Input
                          type="number"
                          min="1"
                    ...
                    <div className="space-y-2">
                      <Label className="text-[#6a7788] font-semibold text-[11px] uppercase tracking-wide">Description</Label>
                      <textarea
                        value={formData.description || ''}
                        onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                        placeholder="Property description..."
                        className="w-full min-h-[96px] rounded-none border border-[#b6bec8] bg-white px-3 py-2 text-[13px] text-[#1f2937] focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-[#F96302]"
                      />
                    </div>

                    <div className="space-y-2">
                      <Label className="text-[#6a7788] font-semibold text-[11px] uppercase tracking-wide">Amenities</Label>
                      <textarea
                ...
                <Card className="border border-[#bcc3cd] bg-[#eef1f4] rounded-none shadow-none">
                  <CardHeader className="pb-4">
                    <CardTitle className="text-lg font-bold text-[#1f2937]">Media</CardTitle>
                    <CardDescription className="text-[#5f6b7c]">Upload or paste a cover image URL.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label className="text-[#6a7788] font-semibold text-[11px] uppercase tracking-wide">Paste Image URL</Label>
                      <Input
                        value={formData.image_url}
                        onChange={handleImageInputChange}
                        placeholder="https://example.com/image.jpg"
              ...
              <div className="xl:col-span-7 space-y-6">
                ...
                {((selectedProperty as any)?.unit_price_breakdown?.length > 0) && (
                  <Card className="border border-[#bcc3cd] bg-[#eef1f4] rounded-none shadow-none overflow-hidden">
                    <CardHeader className="pb-3 border-b border-[#bcc3cd] bg-[#eef1f4]">
                      <CardTitle className="text-lg font-bold text-[#1f2937]">Unit Type Breakdown</CardTitle>
                      <CardDescription className="text-[#5f6b7c]">Exact live pricing tiers from Unit Management.</CardDescription>
                    </CardHeader>
                    <CardContent className="p-0 divide-y divide-[#d4dae2]">
                      {((selectedProperty as any).unit_price_breakdown || []).map((group: any) => (
                        <div key={`edit-group-${group.name}`} className="px-5 py-4 space-y-2">
                          <div className="flex items-center justify-between">
                            <p className="font-semibold text-[#1f2937] uppercase tracking-wide">{group.name}</p>
                            <p className="text-xs font-semibold text-[#5f6b7c]">{Number(group.total_units || 0)} units total</p>
                          </div>
                          <div className="space-y-2">
                            {(group.tiers || []).map((tier: any, tierIndex: number) => (
                              <div key={`edit-${group.name}-tier-${tier.price_per_unit}-${tierIndex}`} className="flex items-center justify-between border border-[#d4dae2] bg-white px-3 py-2">
                                <p className="text-[12px] font-semibold text-[#243041]">Count: {Number(tier.units_count || 0)}</p>
                                <p className="text-[12px] font-bold text-[#1f2937]">KES {(Number(tier.price_per_unit) || 0).toLocaleString()}</p>
                              </div>
                            ))}
                ...
                <Card className="border border-[#bcc3cd] bg-[#eef1f4] rounded-none shadow-none">
                  <CardHeader className="pb-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <CardTitle className="text-lg font-bold text-[#1f2937]">Unit Configuration</CardTitle>
                        <CardDescription className="text-[#5f6b7c]">Define unit types, counts, and rental values.</CardDescription>
                      </div>
                  ...
                  <CardContent className="space-y-3">
                    {formData.units.map((unit: any, index: number) => {
                      ...
                      <div
                        ...
                        className="grid grid-cols-12 gap-3 items-end bg-white p-4 rounded-none border border-[#c7cdd6]"
                      >
                        <div className="col-span-12 md:col-span-5 space-y-1.5">
                          <Label className="text-xs font-semibold text-[#6a7788] uppercase tracking-wide">Unit Type</Label>
                          <Input
                            list={`edit-unit-types-${index}`}
                            placeholder="Select or type..."
                            value={unit.name}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleUnitChange(index, 'name', e.target.value)}
                            className="h-9 rounded-none border border-[#b6bec8] bg-white text-[13px] text-[#1f2937] font-medium"
                          />
                          ...
                          <div className="mt-2 space-y-1.5">
                            ...
                            <div className="flex gap-2">
                              <Input
                                ...
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setUnitSampleUrlDrafts((prev) => ({ ...prev, [sampleDraftKey]: e.target.value }))}
                                placeholder="Paste image URL and click Add"
                                className="h-8 rounded-none border border-[#b6bec8] bg-white text-xs font-medium"
                              />
                              <Button
                                type="button"
                                onClick={() => addUnitSampleImageFromUrl(index, sampleDraftKey)}
                        ...
                        <div className="col-span-4 md:col-span-2 space-y-1.5">
                          <Label className="text-xs font-semibold text-[#6a7788] uppercase tracking-wide">Count</Label>
                          <Input
                            type="number"
                            min="0"
                            value={unit.units_count}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleUnitChange(index, 'units_count', Number(e.target.value))}
                            className="h-9 rounded-none border border-[#b6bec8] bg-white text-sm font-medium text-center"
                          />
                        </div>

                        <div className="col-span-7 md:col-span-4 space-y-1.5">
                          <Label className="text-xs font-semibold text-[#6a7788] uppercase tracking-wide">Rent (KES)</Label>
                          <Input
                            type="number"
                            min="0"
                            value={unit.price_per_unit}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleUnitChange(index, 'price_per_unit', Number(e.target.value))}
                            className="h-9 rounded-none border border-[#b6bec8] bg-white text-sm font-medium text-right"
                          />
                        </div>
            ...
            <Button
              ...
              className="h-9 rounded-none border border-[#154279] bg-[#154279] px-8 text-[11px] font-semibold uppercase tracking-wide text-white hover:bg-[#10335f]"
            >
              {savingProperty ? (
                <>
      ...
      >
        <Card className="border border-[#adb5bf] shadow-sm rounded-none overflow-hidden bg-[#eef1f4]">
          <CardHeader className="border-b border-[#bcc3cd] bg-[#eef1f4] py-4">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div>
                <CardTitle className="text-[#263143] text-[30px] font-bold leading-none flex items-center gap-2">
                    <Building className="h-5 w-5" />
                    Property Directory
                </CardTitle>
                <CardDescription className="text-[#5f6b7c] text-[12px] font-medium mt-1">
                  Managing {safeProperties.length} active properties
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="pt-4 space-y-4 bg-[#eef1f4]">
            {/* Search */}
            <div className="flex flex-col lg:flex-row gap-4 bg-[#d7dee6] p-3 border border-[#c2c9d2]">
              <div className="flex-1 relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#7a8595]" />
                <Input
                  placeholder="Search properties by name, location..."
                  value={searchQuery}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchQuery(e.target.value)}
                  className="h-9 pl-10 border border-[#b6bec8] bg-[#eef1f4] rounded-none text-[13px] text-[#1f2937] placeholder:text-[#778396] focus:border-[#8e98a5]"
                />
              </div>
            </div>
            ...
            {loading ? (
              <div className="flex items-center justify-center py-12">
                <div className="text-center">
                    <Loader2 className="h-8 w-8 text-[#2f3d51] animate-spin mx-auto mb-2" />
                    <p className="text-[#5f6b7c] text-[13px] font-medium">Loading properties...</p>
                </div>
              </div>
            ) : safeProperties.length === 0 ? (
              <div className="text-center py-12 bg-[#eef1f4] border border-dashed border-[#b6bec8]">
                <div className="inline-block mb-3">
                    <Building className="h-8 w-8 text-[#7a8595]" />
                </div>
                <h3 className="text-lg font-bold text-[#334155]">No properties found</h3>
                <p className="text-[#5f6b7c] mt-1 max-w-xs mx-auto text-[13px]">Create your first property to get started.</p>
              </div>
            ) : (
                <div className="space-y-6">
                    {safeProperties.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.location.toLowerCase().includes(searchQuery.toLowerCase())).map((property) => (
                        <Card key={property.id} className="border border-[#cfd6df] rounded-none shadow-none bg-white hover:bg-[#f7f9fb] transition-colors">
                            <CardHeader className="pb-3">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <CardTitle className="text-lg text-[#2d3748]">{property.name}</CardTitle>
                                        <CardDescription className="flex items-center gap-1 mt-1 text-[#5f6b7c]">
                                            <MapPin className="w-3.5 h-3.5 text-[#2fa7bf]" /> 
                                            {property.location} • <span className="text-[#6b7788]">{property.type || 'Apartment'}</span>
                                        </CardDescription>
                                    </div>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        <div>
                                           <p className="text-xs text-[#5f6b7c] font-bold uppercase tracking-wider mb-1">Status</p>
                                           <p className="font-bold text-[#2d3748] text-sm">Active Listing</p>
                                        </div>
                                        <div className="flex flex-wrap gap-2 justify-end">
                                          ...
                                          className="bg-[#2f3d51] hover:bg-[#243041] text-white font-semibold rounded-none border border-[#2f3d51] mb-1 h-9"
                                            >
                                          ...
                                          className="bg-[#156ad8] hover:bg-[#115dbd] text-white font-semibold rounded-none border border-[#156ad8] mb-1 h-9"
                                            >
                                            ...
                                            <Button 
                                              ...
                                              className="bg-[#2fa7bf] hover:bg-[#278ea3] text-white font-semibold rounded-none border border-[#2fa7bf] mb-1 h-9"
                                            >
                                            ...
                                            <Button 
                                                ...
                                                className="bg-[#2dae49] hover:bg-[#24933d] text-white font-semibold rounded-none border border-[#2dae49] mb-1 h-9"
                                            >
                                            ...
                                            <Button 
                                                ...
                                                onClick={() => {
                                                  ...
                                                  className="bg-[#f3bd11] hover:bg-[#d6a409] text-[#1f2937] font-semibold rounded-none border border-[#f3bd11] mb-1 h-9"
                                            >
                                            ...
                                            <Button 
                                                ...
                                                onClick={() => handleDeleteProperty(property.id)} 
                                                  className="bg-[#dc3545] hover:bg-[#c12c3a] text-white font-semibold rounded-none border border-[#dc3545] mb-1 h-9"
                                            >
