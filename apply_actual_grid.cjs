...
const s1 = "        ) : viewMode === 'grid' ? (";
const s2 = "        ) : (\\n            <Card";

const p1 = txt.indexOf(s1);
const p2 = txt.indexOf(s2);

if (p1 > -1 && p2 > -1 && p2 > p1) {
  const newGridBlock = \        ) : viewMode === 'grid' ? (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-10">
    {filteredTenants.map((tenant) => {
      const isActive = tenant.lease_status === 'active';
      return (
        <div key={tenant.id} className="bg-white rounded-[16px] shadow-[0_2px_12px_-4px_rgba(0,0,0,0.08)] border border-slate-100 overflow-visible relative flex flex-col pt-0 transition-all hover:shadow-[0_8px_24px_-8px_rgba(0,0,0,0.12)] h-full">
          {/* Header Banner */}
          ...
          <div className="px-6 flex-1 flex flex-col relative pb-6 pointer-events-none">
             ...
             <div className="pointer-events-auto">
                 ...
                 <div className="flex items-center gap-2 text-[14px] text-slate-500 mb-6">
             ...
             {/* Details list */}
             <div className="space-y-0 text-[14px] mb-6 flex-1 pointer-events-auto">
...
\;
  ...
  txt = txt.substring(0, p1) + newGridBlock + txt.substring(p2);
  ...
  console.log('FAILED TO FIND MATCHES', p1, p2);
}
