-- Fix onboarding invoice visibility for tenants when invoice tenant linkage is inconsistent.
-- This migration does three things:
-- 1) backfills invoices.tenant_id from APPLICANT_ID metadata for onboarding invoices,
-- 2) expands tenant invoice RLS to allow APPLICANT_ID-based ownership,
-- 3) updates finalize_tenant_onboarding_invoice RPC to use the same ownership rule.
