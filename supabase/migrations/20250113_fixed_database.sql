-- COMPLETE DATABASE SETUP FOR REALTORS-LEASERS
-- Run this entire file in Supabase SQL Editor

-- ==================== 1. CREATE CORE TABLES ====================

-- 1.1 Create users table (extends auth.users)
...
-- 1.2 Create user_profiles table
...
-- ==================== 2. CREATE MANAGER-RELATED TABLES ====================

-- 2.1 Create property_managers table
...
-- 2.2 Create manager_assignments table
...
-- 2.3 Create maintenance_requests table
...
-- ==================== 3. CREATE TENANT-RELATED TABLES ====================

-- 3.1 Create leases table
...
-- 3.2 Create tenant_properties table (junction table)
...
CREATE POLICY "Super admins can view all users" ON public.profiles
...
CREATE POLICY "Super admins can do everything with properties" ON public.properties
...
CREATE POLICY "Super admins can manage all assignments" ON public.manager_assignments
  FOR ALL USING (EXISTS (
    SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'super_admin'
  ));
...
-- Insert initial super admin user (replace with your actual user ID)
...
INSERT INTO public.manager_assignments (property_id, manager_id, assigned_by, status)
SELECT 
  p.id,
...
-- Insert tenant_properties relationships
...
INSERT INTO public.maintenance_requests (property_id, tenant_id, title, description, category, priority, status)
SELECT 
  p.id,
  tp.tenant_id,
  CASE 
...
-- Assign maintenance requests to managers
